...
var Sticky = React.createClass({
  ...
  bottom: function() {
    return this.domNode.getBoundingClientRect().bottom;
  },
  /*
  ...
  shouldBeSticky: function() {
    ...
    if(this.originalHeight === undefined) this.originalHeight = this.bottom() - this.top();

    if(this.isSticky) {
      var stickyPoint = totalHeight - this.origin + (this.bottom() - this.top());
    } else {
      this.origin = document.documentElement.scrollHeight - (this.top() + this.originalHeight + this.pageOffset())
      var stickyPoint = totalHeight - this.origin;
    }
    var actualPoint = this.pageOffset() + vHeight;

    if(actualPoint <= stickyPoint) {
      this.isSticky = true;
      return true;
    } else {
      this.isSticky = false;
  ...
  componentDidMount: function() {
    ...
    this.origin = document.documentElement.scrollHeight - (this.bottom() + this.pageOffset());
